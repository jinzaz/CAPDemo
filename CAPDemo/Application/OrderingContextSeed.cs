...
namespace CAPDemo.Application
{
    public class OrderingContextSeed
    {
        public async Task SeedAsync(OrderingContext context, IWebHostEnvironment env, IOptions<OrderingSettings> settings, ILogger<OrderingContextSeed> logger)
        {
            ...
            await policy.ExecuteAsync(async () => {
                ...
                using (context)
                {
                    ...
                    {
                        context.OrderStatus.AddRange(userCustomizationData
        ...
        {
            string csvFileCardTypes = Path.Combine(contentRootPath, "Setup", "CardType.csv");
            ...
            return File.ReadAllLines(csvFileCardTypes)
                 ...
                 .OnCaughtException(ex => { log.LogError(ex, "EXCETPTION ERROR: {Message}", ex.Message); return null; })
                 .Where(x => x != null); 
        }
